...
public class UnityStat : MonoBehaviour
{
	...
	float currHp;
	float currMp;
	private float initHp = 536.0f;
	private float hpGrowth = 92.0f;
	private float initMp = 480.0f;
	private float mpGrowth = 23.5f;
	private float initAtk = 54.2f;
	...
	void RefreshHpMp()
	{
		...
		hpImg.fillAmount = currHp / initHp;
		mpText.text = (int)currMp + " / " + (int)initMp + "  ( + " + initMpRegen + ")";
		mpImg.fillAmount = currMp / initMp;
	}
